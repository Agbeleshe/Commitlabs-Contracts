...
fn test_create_commitment_event() {
    ...
    // For simplicity, we skip full execution here since it requires token setup.
}
...
fn test_allocate_event() {
    ...
    client.initialize(&admin_user, &nft_contract);
    client.add_authorized_allocator(&admin_user, &allocator);
...
fn test_get_total_value_locked_by_asset() {
    ...
    e.as_contract(&contract_id, || {
        let commitment = create_test_commitment(&e, "c_asset1", &owner, 500, 500, 10, 30, 1000);
        set_commitment(&e, &commitment);
        e.storage()
            .instance()
            .set(&DataKey::TotalValueLockedByAsset(asset.clone()), &500i128);
    });
