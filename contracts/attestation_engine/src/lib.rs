...
impl From<AccessControlError> for AttestationError {
    fn from(err: AccessControlError) -> Self {
        match err {
            AccessControlError::NotInitialized => AttestationError::NotInitialized,
            AccessControlError::Unauthorized => AttestationError::Unauthorized,
            AccessControlError::AlreadyAuthorized => AttestationError::Unauthorized, // Still unauthorized if trying to re-auth? Actually better to let caller handle.
            AccessControlError::NotAuthorized => AttestationError::Unauthorized,
            AccessControlError::InvalidAddress => AttestationError::Unauthorized,
        }
    }
}
...
impl AttestationEngineContract {
    ...
    pub fn initialize(
        ...
        AccessControl::init_admin(&e, admin.clone())
            .map_err(|_| AttestationError::AlreadyInitialized)?;
    ...
    /// Add a verifier to the whitelist (admin only)
    pub fn add_verifier(
        e: Env,
        caller: Address,
        verifier: Address,
    ) -> Result<(), AttestationError> {
        // AccessControl library already performs require_admin(caller) which includes require_auth
        match AccessControl::add_authorized_contract(&e, caller, verifier.clone()) {
            Ok(_) | Err(AccessControlError::AlreadyAuthorized) => {}
            Err(err) => return Err(AttestationError::from(err)),
        }

        // Maintain local DataKey::Verifier for backward compatibility or dual-check
        e.storage()
            .instance()
    ...
    /// Alias for add_verifier to maintain compatibility
    pub fn add_authorized_verifier(
        e: Env,
        caller: Address,
        verifier_address: Address,
    ) -> Result<(), AttestationError> {
        Self::add_verifier(e, caller, verifier_address)
    }
    ...
    pub fn remove_verifier(
        ...
        // AccessControl library already performs require_admin(caller) which includes require_auth
        match AccessControl::remove_authorized_contract(&e, caller, verifier.clone()) {
            Ok(_) | Err(AccessControlError::NotAuthorized) => {}
            Err(err) => return Err(AttestationError::from(err)),
        }

        // Remove verifier from local whitelist
        e.storage()
            .instance()
    ...
    /// Alias for remove_verifier to maintain compatibility
    pub fn remove_authorized_verifier(
        e: Env,
        caller: Address,
        verifier_address: Address,
    ) -> Result<(), AttestationError> {
        Self::remove_verifier(e, caller, verifier_address)
    }

    /// Check if an address is an authorized verifier
    fn is_authorized_verifier_internal(e: &Env, address: &Address) -> bool {
        // Check verifier whitelist (local)
        if e.storage()
            .instance()
        ...
        // Check access control (handles admin and authorized contracts)
        AccessControl::is_authorized(e, address)
    }

    /// Check if an address is a verifier (public version)
    pub fn is_verifier(e: Env, address: Address) -> bool {
        Self::is_authorized_verifier_internal(&e, &address)
    }

    /// Alias for is_verifier to maintain compatibility
    pub fn is_authorized_verifier(e: Env, contract_address: Address) -> bool {
        Self::is_verifier(e, contract_address)
    }
        ...
        for att in attestations.iter() {
            if att.attestation_type == String::from_str(&e, "fee_generation") {
                ...
                if let Some(_fee_val) = att.data.get(fee_key.clone()) {
                    ...
                    // In production, fees should be stored as i128 directly
                }
            }
        }
    ...
    ///
    /// Restricted to current admin.
    pub fn update_admin(
        e: Env,
        caller: Address,
        new_admin: Address,
    ) -> Result<(), AttestationError> {
        // Use AccessControl to handle admin check and require_auth
        AccessControl::update_admin(&e, caller.clone(), new_admin.clone())?;

        // Also update local admin key for compatibility
